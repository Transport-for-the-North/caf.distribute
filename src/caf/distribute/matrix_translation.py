...
def run_sectoral(zonal_tripends, tld, translation, zonal_costs, tld_lookup, sec_mat):
    ...
    model = gm.MultiAreaGravityModelCalibrator(
        row_targets=zonal_tripends["o"].values,
    ...
    sec_inputs = furness.SectoralConstraintInputs(
        translation,
        "noham_local_id",
        "noham_base_id",
...
def main(
    ...
    mat = pd.read_csv(target_mat_path, index_col=0, names=range(1, 2771))
    mat = mat.loc[
        np.sort(trans["noham_base_id"].unique()), np.sort(trans["noham_base_id"].unique())
    ].to_numpy()
    ...
    tld_lookup = (
        pd.concat([tld_lookup, missing]).set_index("zone").sort_index().loc[sec_tripends.index]
    )
    tld_lookup.index.name = "zone"
    tld_lookup.reset_index(inplace=True)
    local_lookup = tld_lookup.merge(
        trans, right_on="noham_base_id", left_on="zone", how="inner"
    )[["cat", "noham_local_id"]].sort_values("noham_local_id")
    results, dist = run_sectoral(
        zon_tripends, tlds, trans, zon_costs.values, local_lookup, mat
    )
